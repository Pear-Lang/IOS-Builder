...
def upload_project(repo_name, github_token, project_path, branch, include_patterns=None, exclude_patterns=None, remotes=None, verbose=False):
    ...
    if exclude_patterns:
        # Git does not support excluding patterns directly in git add.
        # Typically, use .gitignore. For the sake of this tool, we'll add all and then remove excluded files.
    ...
    print(Fore.YELLOW + "Running pre-push tests and lint checks...")
    run_command("flutter test", cwd=project_path, verbose=verbose)
    run_command("flutter analyze", cwd=project_path, verbose=verbose)
    ...
    for workflow in workflows:
        ...
        for run in runs:
            try:
                ...
                print(Fore.RED + f"✘ Failed to delete workflow run {run.id}: {e.data['message']}")
            except Exception as e:
...
def main():
    ...
    if action == "createrepo":
        ...
        if not args.skip_upload:
            upload_project(repo_name, github_token, project_path=PROJECT_PATH, branch=BRANCH, include_patterns=INCLUDE_PATTERNS, exclude_patterns=EXCLUDE_PATTERNS, remotes=REMOTES, verbose=args.verbose)
        else:
